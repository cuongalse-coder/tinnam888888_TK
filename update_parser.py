...
pattern1 = re.compile(r'    "customs_declaration": \{.*?\n    \},\n', re.DOTALL)
content = pattern1.sub(customs_declaration_replacement + "\n", content)

# Replace FIELD_MAPPING
...
pattern2 = re.compile(r'FIELD_MAPPING: list\[list\[str\]\] = \[.*?\]', re.DOTALL)
content = pattern2.sub(field_mapping_replacement, content)

with open("utils/parser.py", "w", encoding="utf-8") as f:
    f.write(content)
print("Updated utils/parser.py")
